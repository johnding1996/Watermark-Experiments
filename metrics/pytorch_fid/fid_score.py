...
def get_activations(
    files, model, batch_size=50, dims=2048, device="cpu", num_workers=1, verbose=False
):
...
def compute_statistics_of_path(
    path, model, batch_size, dims, device, num_workers=1, verbose=False
):
    if path.endswith(".npz"):
        ...
        m, s = calculate_activation_statistics(
            files, model, batch_size, dims, device, num_workers, verbose
        )
...
def calculate_fid_given_paths(
    paths, batch_size, device, dims, num_workers=1, verbose=False
):
    ...
    m1, s1 = compute_statistics_of_path(
        paths[0], model, batch_size, dims, device, num_workers, verbose
    )
    m2, s2 = compute_statistics_of_path(
        paths[1], model, batch_size, dims, device, num_workers, verbose
    )
...
def save_fid_stats(paths, batch_size, device, dims, num_workers=1, verbose=False):
    """Calculates the FID of two paths"""
